...
PYTHON_APP_SOURCE="${SWIFTAGENT_PYTHON_APP_SOURCE:-}"
...
if [ -z "$PYTHON_APP_SOURCE" ]; then
  ...
  elif [ -d "$SWIFTAGENT_PACKAGE_DIR/Payloads/Hermes/PythonApp" ]; then
    PYTHON_APP_SOURCE="$SWIFTAGENT_PACKAGE_DIR/Payloads/Hermes/PythonApp"
...
PYTHON_XCFRAMEWORK="${SWIFTAGENT_PYTHON_XCFRAMEWORK:-$SWIFTAGENT_PACKAGE_DIR/Vendor/Python.xcframework}"

if [ ! -d "$PYTHON_APP_SOURCE" ]; then
  echo "SwiftAgent Python app payload was not found at: $PYTHON_APP_SOURCE"
  echo "Set SWIFTAGENT_PYTHON_APP_SOURCE to a directory containing hermes, site-packages, and platform package overlays."
  echo "For SwiftAgent development, run: Scripts/update-hermes.sh"
  exit 1
fi

if [ ! -d "$PYTHON_APP_SOURCE/hermes" ] &&
   [ "$PYTHON_APP_SOURCE_IS_DEFAULT" = "1" ] &&
   [ "${SWIFTAGENT_AUTO_FETCH_HERMES:-YES}" != "NO" ]; then
  echo "SwiftAgent Hermes source is missing; fetching pinned Hermes payload..."
  SWIFTAGENT_HERMES_PAYLOAD_DIR="$PYTHON_APP_SOURCE" "$SCRIPT_DIR/update-hermes.sh"
fi

if [ ! -d "$PYTHON_APP_SOURCE/hermes" ]; then
  echo "SwiftAgent Hermes source was not found at: $PYTHON_APP_SOURCE/hermes"
  echo "Run Scripts/update-hermes.sh, leave SWIFTAGENT_AUTO_FETCH_HERMES enabled, or set SWIFTAGENT_PYTHON_APP_SOURCE to a complete PythonApp payload."
  exit 1
fi

if [ ! -d "$PYTHON_XCFRAMEWORK" ]; then
  echo "SwiftAgent Python.xcframework was not found at: $PYTHON_XCFRAMEWORK"
  echo "Set SWIFTAGENT_PYTHON_XCFRAMEWORK to the vendored Python.xcframework path."
  exit 1
fi
...
case "$EFFECTIVE_PLATFORM_NAME" in
  ...
  -iphoneos)
    ...
    ;;
  *)
    echo "Unsupported platform for SwiftAgent Python packages: $EFFECTIVE_PLATFORM_NAME"
    exit 1
    ;;
esac

DESTINATION="$CODESIGNING_FOLDER_PATH/PythonApp"
echo "Installing SwiftAgent Python payload"
...
HOST_PYTHON=""
if [ -n "${SWIFTAGENT_HOST_PYTHON:-}" ]; then
  HOST_PYTHON_CANDIDATES="$SWIFTAGENT_HOST_PYTHON"
else
...
export PRODUCT_BUNDLE_IDENTIFIER="${PRODUCT_BUNDLE_IDENTIFIER:-com.swiftagent.bundle}"
...
if [ -n "${BUILT_PRODUCTS_DIR:-}" ]; then
  ...
  for FRAMEWORK_NAME in Python ios_system awk dash files shell text; do
    ...
    if [ -d "$FRAMEWORK_SOURCE" ]; then
      echo "Installing SwiftAgent binary framework: $FRAMEWORK_NAME.framework"
      ...
      if [ "${CODE_SIGNING_ALLOWED:-NO}" = "YES" ] && [ "${EXPANDED_CODE_SIGN_IDENTITY:-}" != "-" ] && [ -n "${EXPANDED_CODE_SIGN_IDENTITY:-}" ]; then
        echo "Signing SwiftAgent binary framework: $FRAMEWORK_NAME.framework"
