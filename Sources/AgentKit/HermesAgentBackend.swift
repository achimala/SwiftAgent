...
public final class HermesExtensionProcessBackend: HermesAgentBackend, @unchecked Sendable {
    ...
    private func perform<T>(
        eventHandler: (@Sendable (AgentKitEvent) -> Void)? = nil,
        _ operation: (AgentKitHermesXPCServiceProtocol) throws -> T
    ) throws -> T {
...
private protocol HermesExtensionXPCClient: Sendable {
    func service(
        eventHandler: (@Sendable (AgentKitEvent) -> Void)?
    ) throws -> AgentKitHermesXPCServiceProtocol
}

private final class AgentKitHermesXPCEventSink: NSObject, AgentKitHermesXPCEventSinkProtocol {
    private let eventHandler: (@Sendable (AgentKitEvent) -> Void)?

    init(eventHandler: (@Sendable (AgentKitEvent) -> Void)?) {
        self.eventHandler = eventHandler
    }

    func agentKitHermesDidEmitEvent(kind: String, payload: String) {
        eventHandler?(AgentKitEvent(kind: kind, payload: payload))
    }
}
...
private final class ExtensionFoundationHermesXPCClient: HermesExtensionXPCClient, @unchecked Sendable {
    ...
    private var eventSink: AgentKitHermesXPCEventSink?
    ...
    func service(
        ...
        guard let service = connection.remoteObjectProxyWithErrorHandler({ error in
            NSLog("AgentKit Hermes XPC service error: %@", String(describing: error))
        }) as? AgentKitHermesXPCServiceProtocol else {
    ...
    private func connection(
        ...
        if let connection {
            eventSink = AgentKitHermesXPCEventSink(eventHandler: eventHandler)
        ...
        let createdProcess = try AppExtensionProcess(
            configuration: AppExtensionProcess.Configuration(
                ...
                onInterruption: {
                    NSLog("AgentKit Hermes extension process interrupted")
                }
            )
        )
        let createdConnection = try createdProcess.makeXPCConnection()
        eventSink = AgentKitHermesXPCEventSink(eventHandler: eventHandler)
        createdConnection.remoteObjectInterface = AgentKitHermesXPC.serviceInterface()
        createdConnection.exportedInterface = AgentKitHermesXPC.eventSinkInterface()
