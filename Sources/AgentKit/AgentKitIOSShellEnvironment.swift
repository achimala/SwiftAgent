...
public enum AgentKitIOSShellError: Error, CustomStringConvertible {
    ...
    public var description: String {
        switch self {
        case .missingResources:
            "AgentKit iOS shell resources were not found"
...
public final class AgentKitIOSShellEnvironment: AgentKitShellEnvironment, @unchecked Sendable {
    ...
    public func run(_ command: AgentKitShellCommand) throws -> AgentKitShellResult {
        ...
        let captureDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("AgentKitIOSShell", isDirectory: true)
        ...
        guard let stdoutFile = fopen(stdoutURL.path, "w+") else {
            ...
            throw AgentKitIOSShellError.commandFailedToStart(message)
        }
    ...
    private func prepareRun(cwd: URL, environment: [String: String]) throws {
        ...
        guard FileManager.default.changeCurrentDirectoryPath(cwd.path) else {
            throw AgentKitIOSShellError.workspaceSetupFailed(cwd)
        }
