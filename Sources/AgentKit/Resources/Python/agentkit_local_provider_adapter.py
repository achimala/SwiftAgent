...
def is_agentkit_local_model_base_url(base_url):
...
def ensure_agentkit_model_config(agent_config):
    global _agent_config
    _agent_config = agent_config
    base_url = _agent_config.get("base_url") or ""
    if not is_agentkit_local_model_base_url(base_url):
        return
...
def _agentkit_local_provider_content_to_text(content):
...
def _agentkit_local_provider_messages(messages):
    normalized = []
            ...
            normalized.append(
                {
                    ...
                    "content": _agentkit_local_provider_content_to_text(message.get("content")),
                }
            )
        else:
...
def _agentkit_local_provider_completion_payload(kwargs):
    ...
    request = {
        ...
        "messages": _agentkit_local_provider_messages(kwargs.get("messages", [])),
        "tools": kwargs.get("tools") or [],
        "max_tokens": max_tokens or 256,
        "temperature": temperature,
        "stream": bool(kwargs.get("stream")),
        "agentkit_emit_provider_deltas": False,
    }
...
def _agentkit_local_provider_tool_call_namespace(tool_call, index):
    ...
    return SimpleNamespace(
        id=(tool_call.get("id") if isinstance(tool_call, dict) else None) or f"call_agentkit_local_{uuid.uuid4().hex[:16]}",
...
def _agentkit_local_provider_response(payload, model):
    from types import SimpleNamespace

    text = str(payload.get("final_response") or "")
    tool_calls = [
        _agentkit_local_provider_tool_call_namespace(tool_call, index)
        for index, tool_call in enumerate(payload.get("tool_calls") or [])
        if isinstance(tool_call, dict)
    ] or None

    return SimpleNamespace(
        id=f"chatcmpl-agentkit-local-{uuid.uuid4().hex[:12]}",
        model=model,
        choices=[
            SimpleNamespace(
                index=0,
...
class _AgentKitLocalProviderStream:
                    ...
                    SimpleNamespace(
                        ...
                        delta=SimpleNamespace(
                            ...
                            tool_calls=[
                                SimpleNamespace(
                                    index=tool_index,
                                    id=tool_call.get("id") or f"call_agentkit_local_{uuid.uuid4().hex[:16]}",
...
class _AgentKitLocalProviderChatCompletions:
    def create(self, **kwargs):
        payload = _agentkit_local_provider_completion_payload(kwargs)
        model = kwargs.get("model") or _agent_config["model"]
        if kwargs.get("stream"):
            return _AgentKitLocalProviderStream(payload, model)
        return _agentkit_local_provider_response(payload, model)


class _AgentKitLocalProviderChat:
    def __init__(self):
        self.completions = _AgentKitLocalProviderChatCompletions()


class _AgentKitLocalProviderOpenAI:
    def __init__(self, **kwargs):
        self.api_key = kwargs.get("api_key")
        self.base_url = kwargs.get("base_url")
        self.chat = _AgentKitLocalProviderChat()

    def close(self):
        return None


def configure_agentkit_local_provider_client(run_agent, agent_config):
    global _agent_config
    _agent_config = agent_config
    global _original_run_agent_openai, _original_run_agent_openai_cache

    if is_agentkit_local_model_base_url(_agent_config["base_url"]):
        ...
        run_agent.OpenAI = _AgentKitLocalProviderOpenAI
        run_agent._OPENAI_CLS_CACHE = _AgentKitLocalProviderOpenAI
        return
...
def patch_local_agent_runtime(run_agent, agent_class, agent_config):
    global _agent_config
    _agent_config = agent_config
    if not is_agentkit_local_model_base_url(_agent_config["base_url"]):
        return
    ...
    agent_class._create_request_openai_client = (
        lambda self, reason="", api_kwargs=None: _AgentKitLocalProviderOpenAI(
