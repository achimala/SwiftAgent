...
int create_piped_stdio() {
    if (!(current->files->files[0] = open_fd_from_actual_fd(agentkit_ish_stdin_fd()))) {
        return -1;
    }
    if (!(current->files->files[1] = open_fd_from_actual_fd(agentkit_ish_stdout_fd()))) {
        return -1;
    }
    if (!(current->files->files[2] = open_fd_from_actual_fd(agentkit_ish_stderr_fd()))) {
        return -1;
    }
    return 0;
}
