...
static void *real_tty_read_thread(void *_tty) {
    ...
    char ch;
    for (;;) {
        int err = read(agentkit_ish_stdin_fd(), &ch, 1);
        if (err != 1) {
...
static int real_tty_init(struct tty *tty) {
    ...
    if (ioctl(agentkit_ish_stdin_fd(), TIOCGWINSZ, &winsz) < 0) {
        if (errno == ENOTTY)
            goto notty;
        return errno_map();
    }
    ...
    if (tcgetattr(agentkit_ish_stdin_fd(), &termios) < 0)
        return errno_map();
...
#endif
    if (tcsetattr(agentkit_ish_stdin_fd(), TCSANOW, &termios) < 0)
...
static int real_tty_write(struct tty *tty, const void *buf, size_t len, bool UNUSED(blocking)) {
    if (tty->num != REAL_TTY_NUM)
        return len;
    return write(agentkit_ish_stdout_fd(), buf, len);
}

void real_tty_reset_term() {
    if (!real_tty_is_open) return;
    if (tcsetattr(agentkit_ish_stdin_fd(), TCSANOW, &old_termios) < 0 && errno != ENOTTY) {
