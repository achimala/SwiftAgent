...
static inline int xX_main_Xx(int argc, char *const argv[], const char *envp) {
    ...
    struct bind_spec {
        char linux_path[MAX_PATH + 1];
        char host_path[MAX_PATH + 1];
        bool read_only;
    } binds[16];
    int bind_count = 0;

    while ((opt = getopt(argc, argv, "+r:f:d:c:n:b:")) != -1) {
        switch (opt) {
            case 'r':
            case 'f':
                root = optarg;
            ...
            case 'b': {
                ...
                if (separator == NULL || separator == optarg || separator[1] == '\0') {
                    fprintf(stderr, "invalid bind mount: %s\n", optarg);
                    exit(1);
                }
                size_t linux_len = (size_t)(separator - optarg);
                if (linux_len >= sizeof(binds[bind_count].linux_path) ||
                        strlen(separator + 1) >= sizeof(binds[bind_count].host_path)) {
                    fprintf(stderr, "bind mount path too long: %s\n", optarg);
                    exit(1);
                }
                memcpy(binds[bind_count].linux_path, optarg, linux_len);
                binds[bind_count].linux_path[linux_len] = '\0';
                strcpy(binds[bind_count].host_path, separator + 1);
                binds[bind_count].read_only = false;
                bind_count++;
                break;
            }
        }
    ...
    for (int bi = 0; bi < bind_count; bi++) {
        if (fakefs_bind_mount(binds[bi].linux_path, binds[bi].host_path, binds[bi].read_only) < 0) {
            fprintf(stderr, "bind mount failed: %s=%s\n", binds[bi].linux_path, binds[bi].host_path);
            return 1;
        }
    }
